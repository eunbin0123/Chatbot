...
export default function Admin({ chatbotType }) {
    ...
    if (mcpList.length > 0 && !isInitialMcpLoaded) {
      ...
      mcpList.forEach((mcp) => {
        ...
        for (let dir of savedDirs) {
          ...
          if (existingIdx >= 0) { dir.items[existingIdx] = mappedItem; found = true; break; }
        }
        if (!found && savedDirs[0]) savedDirs[0].items.push(mappedItem);
      });
  ...
  const toggleItemActive = (dirId, itemId, e) => {
    e.stopPropagation();
    updateMcpData(mcpDirectories.map((dir) => dir.id === dirId
  ...
  const promptTagOptions = [
    { id: "no_politics", label: "정치 언급 금지" }, { id: "no_religion", label: "종교 언급 금지" },
    { id: "no_social_controversy", label: "사회적 논란 언급 금지" }, { id: "no_profanity", label: "비속어 및 혐오 표현 금지" },
    { id: "no_competitors", label: "경쟁사 언급 금지" }, { id: "no_personal_info", label: "개인정보 요구 금지" },
  ...
  const confirmReissueKey = () => {
    ...
    const today = new Date();
    const dateStr = `${today.getFullYear()}-${String(today.getMonth() + 1).padStart(2, "0")}-${String(today.getDate()).padStart(2, "0")}`;
    setApiKeys((prev) => prev.map((k) => k.id === reissueTargetId ? { ...k, value: newKey, date: dateStr } : k));
    setReissueTargetId(null); setAlertMessage("API 키 재발급 요청이 정상적으로 접수되었습니다.");
  };
  ...
  const handleCopyEmbedCode = () => {
    try {
      const ta = document.createElement("textarea"); ta.value = getEmbedCode(); document.body.appendChild(ta); ta.select(); document.execCommand("copy"); document.body.removeChild(ta);
  ...
  const sidebarStages = [
    ...
    {
      id: "knowledge_group", label: "02 지식", isGroup: true,
      subItems: [
        { id: "rag", label: "RAG 설정", engine: stageStatus.rag ? stageEngines.rag : "OFFLINE" },
        { id: "mcp", label: "MCP 설정", engine: `${mcpDirectories.filter((d) => d.active).reduce((sum, d) => sum + d.items.length, 0)} UNITS ACTIVE` },
        { id: "prompt", label: "프롬프팅 설정", engine: "활성화 되었습니다." },
      ],
    },
                    ...
                    {sidebarStages.map((stage) => {
                      ...
                      return (
                        <div key={stage.id} className={`setting-card ${isActive ? "active-no-glow" : ""}`}
                          style={{ padding: "16px 20px", cursor: "pointer", borderRadius: "16px", border: isActive ? "1.5px solid var(--accent)" : "1px solid var(--border-glass)", backgroundColor: isActive ? "rgba(0,198,255,0.08)" : "rgba(255,255,255,0.02)", transition: "all 0.2s ease-out", marginBottom: "4px" }}
                              ...
                              <div key={dir.id} className="setting-card glass-card" style={{ padding: 0, overflow: "hidden", border: "1px solid var(--border-glass)", transition: "all 0.3s" }}>
                                <div onClick={() => toggleDirectory(dir.id)} style={{ padding: "20px", display: "flex", justifyContent: "space-between", alignItems: "center", cursor: "pointer", borderBottom: "1px solid var(--border-glass)", background: "rgba(255,255,255,0.02)" }}>
                                  ...
                                  <div style={{ display: "flex", alignItems: "center", gap: "16px" }}>
                                    <button onClick={(e) => handleDeleteDirectory(dir.id, e)} style={{ background: "none", border: "none", color: "#718096", cursor: "pointer", padding: "4px", display: "flex", alignItems: "center" }}>
                                    ...
                                    <div onClick={(e) => toggleDirectoryActive(dir.id, e)} style={{ width: "36px", height: "18px", backgroundColor: dir.active ? "var(--accent)" : "#1e293b", borderRadius: "18px", position: "relative", cursor: "pointer" }}>
                            ...
                            <div>
                              ...
                              <div style={{ border: "1px solid #2d3748", borderRadius: "14px", padding: "12px", display: "grid", gridTemplateColumns: "1fr 1fr", gap: "8px" }}>
                                {promptTagOptions.map((tag) => {
                                  ...
                                  return (
                                    <div key={tag.id} onClick={() => togglePromptTag(tag.id)}
                                      style={{ padding: "8px 12px", borderRadius: "10px", border: "1px solid " + (isActive ? "var(--accent)" : "#2d3748"), backgroundColor: isActive ? "rgba(0,198,255,0.15)" : "transparent", color: isActive ? "#fff" : "var(--text-muted)", fontSize: "12px", fontWeight: "600", display: "flex", alignItems: "center", justifyContent: "center", gap: "6px", cursor: "pointer" }}>
                                ...
                                {customTags.map((tag) => {
                                  ...
                                  return (
                                    <div key={tag.id} onClick={() => togglePromptTag(tag.id)}
                                      style={{ position: "relative", padding: "8px 12px", borderRadius: "10px", border: "1px solid " + (isActive ? "var(--accent)" : "#2d3748"), backgroundColor: isActive ? "rgba(0,198,255,0.15)" : "transparent", color: isActive ? "#fff" : "var(--text-muted)", fontSize: "12px", fontWeight: "600", display: "flex", alignItems: "center", justifyContent: "center", gap: "6px", cursor: "pointer" }}>
                                      {isActive && <span>✓</span>}{tag.label}
                                      <button type="button" className="prompt-tag-remove" onClick={(e) => handleRemoveCustomTag(e, tag.id)} style={{ position: "absolute", right: "8px" }}>
                                ...
                                <div style={{ gridColumn: "span 2", padding: "1px", borderRadius: "12px", border: "1px solid #4a5568", background: "rgba(255,255,255,0.02)" }}>
                                  <div style={{ display: "flex", alignItems: "center" }}>
                                    <input type="text" placeholder="+ 태그 직접 입력 후 Enter"
      ...
      <div id="chatbot-wrapper">
        {chatbotType === "sdk" ? (
          ...
          <BasicChatbot
            ...
            analysisApiKey={resolvedAnalysisApiKey}
            ragApiKey={resolvedRagApiKey}
            responseApiKey={resolvedResponseApiKey}
